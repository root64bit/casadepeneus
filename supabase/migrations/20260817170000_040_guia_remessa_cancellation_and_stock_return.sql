...
BEGIN
  ...
  v_actor_id := COALESCE(auth.uid(), v_doc.updated_by, v_doc.created_by, (SELECT id FROM user_profiles LIMIT 1));
  ...
  VALUES(v_doc.company_id, v_doc.id, v_doc.status, 'REVERSED', TRIM(p_reason), v_actor_id);
