...
interface SaleDocumentHistoryProps {
  ...
  onCancel?: (document: SaleInvoice) => void;
}

export const SaleDocumentHistory: React.FC<SaleDocumentHistoryProps> = ({
  documents,
  guideOnly,
  operatorName,
  onPrint,
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {documents.slice(0, 15).map((document) => {
              const isCancelled =
                ...
                String(document.status).toUpperCase() === 'REVERSED';
              return (
                <tr
                  key={document.id || document.docNumber}
                ...
                >
                  ...
                  <td className="p-2">
                    <span className="inline-block px-2 py-0.5 rounded text-[10px] font-extrabold uppercase bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">
                      {document.documentTypeCode === 'CUSTOMER_DELIVERY_NOTE' || document.docNumber?.startsWith('GR')
                        ? 'Guia de Remessa'
                        : document.documentTypeCode === 'CASH_SALE'
                        ? 'VD'
                        : 'Factura'}
                    </span>
                  </td>
                  ...
                  <td className="p-2 text-slate-600 dark:text-slate-400">
                    {document.operatorName || operatorName || 'Operador'}
                  </td>
                  <td className="p-2 text-center">
                        ...
                        <>
                          ...
                          {onCancel && (
                            <button
                              ...
                              className="inline-flex items-center space-x-1 px-2.5 py-1 bg-red-700 text-white rounded font-bold text-[11px] hover:bg-red-800 transition-all shadow-sm"
                            >
